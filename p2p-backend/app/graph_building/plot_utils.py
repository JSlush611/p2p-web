...
def plot_average_time_by_year(df):
    ...
    trace1 = go.Scatter(x=avg_time.index.tolist(), y=avg_time.tolist(), mode='lines+markers', name='Average Time', line=dict(color='orange'))
...
def plot_gender_distribution_by_year(df):
    ...
    trace1 = go.Bar(x=gender_distribution.index.tolist(), y=gender_distribution['M'].tolist(), name='Male', marker=dict(color='#1f77b4'))
...
def plot_median_time_by_year(df):
    ...
    trace1 = go.Scatter(x=median_time.index.tolist(), y=median_time.tolist(), mode='lines+markers', name='Median Time', line=dict(color='darkgreen'))
...
def plot_user_time_percentile_by_year(df, name):
    ...
    trace1 = go.Scatter(x=years, y=percentiles_all, mode='lines+markers', name='Overall Percentile', line=dict(color='blue'))
...
def plot_average_time_by_gender_age_group_and_year(df, gender, age_group, name, overlay_avg_time=False, overlay_user_time=False):
    ...
    trace1 = go.Scatter(x=avg_time.index.tolist(), y=avg_time.tolist(), mode='lines+markers', name=f'Average Time ({gender}, {age_group})', line=dict(color='blue'))
    traces = [trace1]

    if overlay_avg_time:
        overall_avg_time = df.groupby('Year')['Time (min)'].mean()
        trace2 = go.Scatter(x=overall_avg_time.index.tolist(), y=overall_avg_time.tolist(), mode='lines', name='Overall Average Time', line=dict(color='red', dash='dash'))
    ...
    if overlay_user_time and name:
        ...
        if not user_data.empty:
            ...
            trace3 = go.Scatter(x=user_time_by_year.index.tolist(), y=user_time_by_year.tolist(), mode='lines+markers', name=f"{name}'s Time", line=dict(color='green'))
